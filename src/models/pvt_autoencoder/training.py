...
def train_pvt_autoencoder_frozen(
    model: nn.Module,
    ...
    """
    Strategy 1: Keep encoder frozen throughout training (train decoder + bottleneck only).
    """
    model.to(device)
...
def train_pvt_autoencoder_progressive_unfreeze(
    model: nn.Module,
    train_loader: DataLoader,
    val_loader: DataLoader,
    epochs: int,
    ...
    if verbose_validation:
        ...
        logger.info(
            f"Encoder frozen until epoch {unfreeze_epoch}, then unfrozen with reduced LR for encoder"
        )
...
def train_pvt_autoencoder_differential_lr(
    model: nn.Module,
    train_loader: DataLoader,
    val_loader: DataLoader,
    epochs: int,
    ...
    for epoch in range(epochs):
        ...
        if verbose_training:
            logger.info(
                f"[Epoch {epoch + 1}/{epochs}] Diff-LR | Train: {avg_train_loss:.4f} | Val: {avg_val_loss:.4f}"
            )
